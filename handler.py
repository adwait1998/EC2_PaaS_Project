...
accesskey1 = str(os.getenv("ac1"))
accesskey2 = str(os.getenv("ac2"))
REGION = "us-east-1"
s3Client = boto3_client('s3', aws_access_key_id=accesskey1, aws_secret_access_key=accesskey2, region_name=REGION)
dbClient = boto3_client('dynamodb', aws_access_key_id=accesskey1, aws_secret_access_key=accesskey2, region_name=REGION)
...
def face_recognition_handler(event, context):	
	...
	try:
		...
		s3Client.download_file(input_bucket, input_file_name, filepath)
	...
	response = s3Client.upload_file("/tmp/"+x, output_bucket, object_name)
